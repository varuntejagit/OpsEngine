from ops.core.base_data_reader import DataReader
...
class SparkReader(DataReader):
    ...
    def __init__(self, spark, platform, config):
        """
    ...
    def read_data(self):
        ...
        Extract options, schema, input path, and file format from the configuration.
        ...
        if sql_config:
            ...
            if file_format == "jdbc" and self.platform == "databricks":
                options = {**options, **JDBCUtils.get_database_credentials(self.spark, options, self.platform)}
