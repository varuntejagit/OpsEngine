...
from ops.core.definitions import DQDefaults, DQFunctionSpec
...
class Validator(object):
    ...
    def tag_source_with_dq(
            cls, spark: SparkSession,source_pk: List[str], source_df: DataFrame, results_df: DataFrame
    ) -> DataFrame:
        ...
        if (failures_df.limit(1).count()) > 0:
            run_success = False
        ...
        return cls._join_complementary_data(
            spark, run_name, run_success, raised_exceptions, source_df
        )
    ...
    def _join_complementary_data(
            spark: SparkSession,run_name: str, run_success: bool, raised_exceptions: bool, source_df: DataFrame
    ) -> DataFrame:
        ...
        complementary_df = spark.createDataFrame(
