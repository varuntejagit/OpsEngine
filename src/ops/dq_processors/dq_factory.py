...
class DQFactory(object):
    ...
    @classmethod
    def run_dq_process(cls, spark: SparkSession, dq_spec: DQSpec, data: DataFrame) -> DataFrame:
        ...
        for expectation in DQDefaults.CUSTOM_EXPECTATION_LIST.value:
            importlib.__import__(
                "ops.dq_processors.custom_expectations." + expectation
            )

        """
            The get_context function in Great Expectations is used to create or load a data context. 
            A data context is the main object in Great Expectations that holds 
            configurations, expectations, and validations for your data.
        """
        ...
        if dq_spec.dq_type == DQType.VALIDATOR.value:
            ...
            results, results_df = cls._configure_and_run_checkpoint(
                spark, dq_spec, context, batch_request, expectation_suite_name, source_pk
            )

            cls._write_to_result_sink(spark, dq_spec, results_df)
            ...
            if (
                ...
                data = Validator.tag_source_with_dq(spark, source_pk, data, results_df)
        else:
            raise TypeError(
    ...
    def _configure_and_run_checkpoint(
        ...
        return results, cls._transform_checkpoint_results(
            spark, results.to_json_dict(), dq_spec
        )
    ...
    def _get_data_context_config(cls, dq_spec: DQSpec) -> DataContextConfig:
        ...
        the file system (e.g., local file system) or S3, meaning that the DQ artifacts
        ...
        Returns:
            DataContextConfig: Configuration object for the data context.
        """
        # Define the type for store_backend, which can be either a FilesystemStoreBackendDefaults or
        # S3StoreBackendDefaults
        store_backend: Union[FilesystemStoreBackendDefaults, S3StoreBackendDefaults]
        data_docs_site = None

        # Check if the store backend is set to local file system
        if dq_spec.store_backend == DQDefaults.FILE_SYSTEM_STORE.value:
            # Configure for local file system backend
            """This class is used to define default configurations for storing Great Expectations artifacts (like 
            expectations, validations, and checkpoints) on a local file system. It sets up the necessary directory 
            structure and paths for storing these artifacts locally """
            store_backend = FilesystemStoreBackendDefaults(
                root_directory=dq_spec.local_fs_root_dir
            )
            # Get data docs site configuration for local file system
            data_docs_site = cls._get_data_docs_sites(
                "local_site", store_backend.data_docs_sites, dq_spec
            )
        # Check if the store backend is set to S3
        elif dq_spec.store_backend == DQDefaults.FILE_SYSTEM_S3_STORE.value:
            # Configure for S3 backend
            """This class is used to define default configurations for storing Great Expectations artifacts in Amazon 
            S3. It provides configuration options such as bucket name, prefixes for different types of artifacts, 
            and other S3-related settings. """
            ...
            # Get data docs site configuration for S3
            data_docs_site = cls._get_data_docs_sites(
                "s3_site", store_backend.data_docs_sites, dq_spec
            )

        # Return the DataContextConfig with the specified store backend and data docs site configurations
        """This is a configuration class that defines the overall setup for a Great Expectations data context. It 
        includes various settings, such as the store backend (local filesystem or S3), data docs sites configuration, 
        and whether anonymous usage statistics are enabled. """
    ...
    def _transform_checkpoint_results(
            cls, spark: SparkSession, checkpoint_results: dict, dq_spec: DQSpec
    ) -> DataFrame:
    ...
    def _write_to_result_sink(
        ...
        if dq_spec.result_sink_db_table or dq_spec.result_sink_location:
            ...
            config = {
                "dataframe_options": options,
                "path": dq_spec.result_sink_location,
                "table_name": dq_spec.result_sink_db_table,
                "partitionBy": partitions,
                "mergeSchema": True,
                "format": dq_spec.result_sink_format
            }
            SparkWriter(spark, df, config).write_data()
